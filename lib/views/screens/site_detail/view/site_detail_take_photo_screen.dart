...
class SiteDetailsTakePhoto extends StatefulWidget {
  const SiteDetailsTakePhoto({Key? key}) : super(key: key);

  @override
  State<SiteDetailsTakePhoto> createState() => _SiteDetailsTakePhotoState();
}

class _SiteDetailsTakePhotoState extends State<SiteDetailsTakePhoto>
  ...
  void initState() {
    ...
    _controller.areaList.clear();
    _controller.floorList.clear();

    for (var element in _controller.floorPlan) {
      log(element.floorData?.length.toString() ?? "Null",
          name: "Floor floorData");

      if (element.floorData?.isNotEmpty ?? false) {
        log(element.areaName ?? "Null", name: "Floor areaName");
        _controller.areaList.add(element.areaName ?? '');
      }
    }

    _controller.floorPlan[_controller.selectedAreaIndex.value].floorData
        ?.forEach((element) {
      _controller.floorList.add(element.floorName ?? '');
      log(element.floorName ?? '', name: "Floor floorName");
    });
                ...
                Obx(
                  () => Expanded(
                    child: DropdownButtonFormField(
                      ...
                      onChanged: (String? areas) {
                        if (areas != null) {
                          _controller.addFloorSelectedAreaValue.value = areas;
                          _controller.floorList.clear();
                          for (var element in _controller.floorPlan) {
                            if (element.areaName == areas) {
                              element.floorData?.forEach((element) {
                                _controller.floorList
                                    .add(element.floorName ?? '');
                              });
                            }
                          }

                          _controller.addFloorSelectedValue.value =
                              _controller.floorList[0];
                        }
                      },
                ...
                Expanded(
                  child: Obx(
                    () => DropdownButtonFormField(
                      ...
                      isDense: true,
                      dropdownColor: AppColors.whiteColor,
                      value: _controller.addFloorSelectedValue.value,
                      onChanged: (String? floorData) {
                        if (floorData != null) {
                          _controller.addFloorSelectedValue.value = floorData;
                        }
                      },
                      icon: const Icon(Icons.expand_more_rounded),
                      items: _controller.floorList
                          .map<DropdownMenuItem<String>>((String value) {
                        return DropdownMenuItem<String>(
                          value: value,
                          child: Text(
                ...
                onTap: () async {
                  ...
                  if (status) {
                    ...
                    if (file != null) {
                      ...
                      _controller.capturedImageUrl.value = file.path;
              ...
              GestureDetector(
                onTap: () async {
                  ...
                  try {
                    ...
                    if (!mounted) return;
                    _controller.capturedImageUrl.value = image.path;
